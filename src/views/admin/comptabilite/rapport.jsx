...
import { StyleSheet, Text, View, TouchableOpacity, Keyboard, TouchableWithoutFeedback, Dimensions } from "react-native";
...
export default function Rapport(){
    ...
    const [choices, setChoices] = useState(
        [
            //{ name: 'les statistiques', id: '1' },
            { name: 'la gestion des montants de scolarité', id: '2' },
            { name: 'les impayés', id: '3' }
        ]
    );
    ...
    function handleSend(lieu){
        setChoice(lieu);
        switch (lieu) {
            case "1":
                nav.navigate("admin/statistique");
                break;
            case "2":
                nav.navigate("admin/montant");
                break;
            case "3":
                nav.navigate("admin/impaye");
                break;
            default:
                break;
        }
    ...
    const style = StyleSheet.create({
        ...
        block:{marginLeft: 20, padding: 10, marginRight: 20, marginTop: 1, alignItems: "center", justifyContent: "center", flex:1}
    });
    ...
    return (
        <View style={style.container}>
            ...
            <View style={style.block}>
                ...
                <DropdownSelect
                    label="Veuillez choisir une action à réaliser" placeholder="Que voulez vous faire"
                    options={choices} optionLabel={'name'}
                    optionValue={'id'} selectedValue={choice}
                    onValueChange={(itemValue) => handleSend(itemValue)}
                    dropdownErrorStyle={{
                        borderColor: 'red', borderWidth: 2, borderStyle: 'solid',
                    }}
                    dropdownErrorTextStyle={{ color: 'red', fontWeight: '500' }}
                    error={choice ? '' : 'faite un choix svp'} primaryColor={'green'}
                />
