...
export default function Impaye(){
    ...
    useMemo(()=>{
        Get("/inscription/all/ecole/"+user?.ecole?._id+"/"+user?.ecole?.anneeScolaire, token).then(
            (rs)=>{
                if(!rs?.error){
                    const insc= rs?.inscriptions?.filter((i)=> i?.paye<=i?.classe?.scolarite);
    ...
    const style = StyleSheet.create({
        ...
        box: {
            backgroundColor: '#87CEEB', height: 100,
        },
        bottomWavy: {
        ...
        <View style={style.container}>
            <View style={style.top}>
                <View style={style.box}>
                    <Svg 
                        height={280} width={Dimensions.get('screen').width}
            ...
            <View style={style.part2}>
                ...
                <ScrollView>
                    {
                        inscriptions?.filter((ins)=> ins?.reglement?.includes(filtre)).map(
                            (i, idx)=>{
                                return (
                                    <View style={{marginLeft: -25, marginRight: -25}} key={idx}>
                                        <CardThree title={i?.eleve?.nom+" "+i?.eleve?.prenom} icon={"forward"} iconColor={"grey"}
                                            subTitle={i?.classe?.libelle+ " (montant dû : " + (i?.classe?.scolarite-i?.paye)+")"} 
